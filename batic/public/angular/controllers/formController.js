            ...
            save: function ($event) {
                ...
                Doctype.save(capitalizeText($scope.$resolve.doctype.replace(/-/g, ' ')), baticApp.doctypes[$scope.$resolve.doctype].data($rootScope)).then(function (response) {
                    if (response.status === 200) {
                        ...
                        $state.transitionTo('app.form', {parent: $scope.$resolve.parent, doctype: $scope.$resolve.doctype, type: 'view', docname: response.data.data.name});
                    } else {
                        NProgress.done();
...
<!--               </div>-->
               ...
               <div class="input-group">
                  <div class="form-control" ng-model="ngModel" id="{{field.name}}" data-fieldname="{{field.name}}" class="form-control" ng-class="(sizeSmall=='true')?'form-control-sm':''"  data-fieldtype="{{field.type}}"> 
                    ...
                    <span ng-if="!ngModel.length" class="text-secondary">${baticApp.app.__lang('No file selected')}</span>
                  </div>
                  ...
                  <button class="btn btn-secondary add-attach-image" ng-class="(sizeSmall=='true')?'btn-sm':''" type="button" tooltip="${baticApp.app.__lang('Attach')}">
                        ...
                        <i ng-if="field.type=='Attach File' || field.type=='Attach'" class="far fa-paperclip"></i>
                  </button>
               </div>
           </div>
        `,
...
}).directive("fieldMultiSelect", function ($rootScope, $timeout) {
    return {
        restrict: 'E',
        template: `
            <div class="form-field-multi-select dropdown" ng-hide="hideField==true" data-doctype="{{field.link.doctype}}" data-reference_doctype="{{field.link.reference_doctype}}" data-filters="{{field.link.filters}}" data-ignore_user_permissions="{{field.link.ignore_user_permissions}}" data-query="{{field.link.query}}"> 
              <label for="{{field.name}}" ng-if="hideLabel!='true'" class="form-control-label" ng-class="(field.required)?'reqd':''">{{ field.title }}</label>
              <ng-dropdown-multiselect options="selectOptions" selected-model="ngModel" extra-settings="extraSettings" translation-texts="{buttonDefaultText:'${$rootScope.__lang('Select')} '+field.title}"></ng-dropdown-multiselect>
            </div>
        `,
        link: function (scope, element, attrs) {
            let link = {}
            let link_field = frappe.get_meta(scope.field.link.doctype).fields.filter(f => f.in_list_view && f.fieldtype === 'Link')[0];
            if (link_field) {
                link['doctype'] = link_field.options;
                scope.selectFieldName = link_field.fieldname;
                scope.extraSettings = {smartButtonMaxItems: 10, enableSearch: true, displayProp: link_field.fieldname, idProp: link_field.fieldname, externalIdProp: link_field.fieldname}
            }
            $timeout(() => {
                link['reference_doctype'] = element.find('.form-field-multi-select').attr('data-reference_doctype');
                link['ignore_user_permissions'] = (element.find('.form-field-multi-select').attr('data-ignore_user_permissions')) ? element.find('.form-field-multi-select').attr('data-ignore_user_permissions') : 0;
                if (element.find('.form-field-multi-select').attr('data-filters')) {
                    $timeout(() => scope.filters = element.find('.form-field-multi-select').attr('data-filters'), 50);
                    link['filters'] = element.find('.form-field-multi-select').attr('data-filters');
                } else {
                    $timeout(() => scope.filters = null, 50);
                }
                scope.selectOptions = [];
                scope.$root.fieldLink(link, '', function (response) {
                    if (response.status === 200) {
                        if (response.data.message && response.data.message.length) {
                            response.data.message.map(row => {
                                row[scope.selectFieldName] = row.value;
                                scope.selectOptions.push(row);
                            });
                        }
                        scope.fieldIsLoading = false;
                    }
                });
            }, 200);
            element.on('input', '.form-field-multi-select .dropdown-header > input.searchField', function () {
                let txt = $(this).val()
                let selectOptions = [];
                scope.$root.fieldLink(link, txt, function (response) {
                    if (response.status === 200) {
                        if (response.data.message && response.data.message.length) {
                            response.data.message.map(row => {
                                row[scope.selectFieldName] = row.value;
                            ...
                            let selectOptions_old = angular.copy(scope.selectOptions);
                            const uniqueById = [...selectOptions_old, ...selectOptions].filter((item, index, self) =>
                                index === self.findIndex((obj) => obj.category === item.category)
                            );
                            scope.selectOptions = uniqueById;
                        }
                        scope.fieldIsLoading = false;
                    }
                });
            });
            element.on('blur', '.form-field-multi-select .dropdown-header > input.searchField', function () {
